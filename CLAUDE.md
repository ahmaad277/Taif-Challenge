...
- **Persists to a shared server** (`/api/content.js`, a Vercel serverless function backed by **Vercel Blob**) so custom content syncs across **all devices** on the domain. All records live in a single public **`content.json`** blob (images as base64); **IndexedDB** (`taifContent` DB, `items` store) is an **offline cache + fallback**. Three record kinds: `add` (custom item), `edit` (`{ kind:'edit', targetKey, fields }` override of a built-in), `delete` (`{ kind:'delete', targetKey }` reversible hide).
  - **Sync flow:** `GET /api/content` returns the blob's **public URL** (not the data — the serverless response is size-limited but the content with images isn't), and `serverAll()` fetches `content.json` straight from the Blob CDN. `POST`/`DELETE` do a server-side read-modify-write of the blob (upsert/remove by `id`). `dbAll()` mirrors the server into the cache and *preserves* cache-only items (added before sync) so they aren't lost — they show a migrate button (`migrateLocal()`). If the server is unreachable, the editor falls back to the cache with an offline banner.
  - **Auth/env** (server): `BLOB_READ_WRITE_TOKEN` is injected automatically by the connected Vercel Blob store — no manual env setup. Writes require a host passcode (`x-write-key` header, prompted once, stored in localStorage); it's `CONTENT_WRITE_PASSCODE` env or **`'taif'` by default** (hardcoded fallback — visible in the public repo, so a soft deterrent, not real auth). Without the Blob token the API returns 503 and the client goes local-only. Requires the `@vercel/blob` dependency (installed by Vercel for the function).
